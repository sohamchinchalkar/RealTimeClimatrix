...
// Function to manage screen responsiveness
function manageResponsiveDimensions() {
    const screenWidth = window.innerWidth;
    const screenHeight = window.innerHeight;

    // Set a base aspect ratio for the map
    const aspectRatio = 1480 / 690;

    // Calculate new dimensions that maintain the aspect ratio
    const newWidth = Math.min(screenWidth * 0.95, 1480); // Scale down to fit within 95% of the screen width
    const newHeight = Math.min(newWidth / aspectRatio, screenHeight * 0.8); // Scale height based on aspect ratio and fit within 80% of screen height

    // Update the SVG dimensions
    const svg = d3.select("svg");
    svg.attr("width", newWidth).attr("height", newHeight);

    // Update map projection to fit the new dimensions
    const newProjection = d3.geoMercator()
        .scale(newWidth / 5) // Adjust scale relative to new width
        .translate([newWidth / 2, newHeight / 1.5]); // Center map in the new space
    ...
    // Update city bubble positions
    d3.selectAll(".city")
        .attr("cx", d => {
            const projected = newProjection([+d.lng, +d.lat]);
            return projected ? projected[0] : null;
        })
        .attr("cy", d => {
            const projected = newProjection([+d.lng, +d.lat]);
    ...
    // Dynamically position the search container
    d3.select("body > div:nth-child(3)") // Assuming the search container is the 3rd div
        .style("right", `${Math.max(20, screenWidth * 0.02)}px`) // Scale padding proportionally
        .style("top", `${Math.max(20, screenHeight * 0.02)}px`);

    // Dynamically position the legend card
    d3.select(".legend-card")
        .style("top", `${Math.max(100, screenHeight * 0.12)}px`) // Adjust top position dynamically
        .style("right", `${Math.max(30, screenWidth * 0.02)}px`);
}

// Call the responsive function initially
manageResponsiveDimensions();

// Add event listener to update dimensions on window resize
window.addEventListener("resize", manageResponsiveDimensions);
...
async function initializeBubbles() {
    ...
    results.forEach((data, i) => {
        if (data && data.cod === 200) {
            ...
            // Get the current projection (dynamic based on screen size)
            const currentProjection = d3.geoMercator()
                .scale(d3.select("svg").attr("width") / 5) // Adjust scale dynamically
                .translate([d3.select("svg").attr("width") / 2, d3.select("svg").attr("height") / 1.5]);

            // Define initial position (off-screen left)
            const initialX = -4000;

            // Get the final x and y positions from the updated projection
            const projected = currentProjection([+city.lng, +city.lat]);
            const finalX = projected ? projected[0] : null;
            const finalY = projected ? projected[1] : null;
...
// Reinitialize the bubbles when resizing to maintain their alignment
